...
// ===== Các phần đã tách ra khỏi App.jsx (Bước 0 — xem đề xuất tách kiến trúc) =====
...
const DashboardScreen = ({ user, onLogout }) => {
  ...
  // ===== Tab Bếp (KDS) — toàn bộ state/logic đã chuyển sang hook useKitchen (Bước 1) =====
  const {
    kitchenItems,
    visibleKitchenItems,
      ...
      <main style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '32px' }}>
          ...
          <KitchenTab
            ...
            setKitchenViewMode={setKitchenViewMode}
            kitchenCategoryFilter={kitchenCategoryFilter}
            setKitchenCategoryFilter={setKitchenCategoryFilter}
...
const App = () => {
  ...
  // Chỉ hiện màn chào 1 lần cho tới khi đăng xuất — tránh hiện lại mỗi lần currentUser re-render
  // (vd. các chỗ khác trong Dashboard set lại currentUser với object mới nhưng cùng 1 phiên đăng nhập).
  useEffect(() => {
    if (currentUser && !welcomeShownRef.current) {
  ...
  return (
    <AnimatePresence mode="wait">
      {showWelcome ? (
        <WelcomeSplash key="welcome" user={currentUser} />
      ) : currentUser ? (
        <DashboardScreen key="dashboard" user={currentUser} onLogout={() => { welcomeShownRef.current = false; setCurrentUser(null); }} />
      ) : (
